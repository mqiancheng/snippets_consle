...
// --- 基础配置 (仅保留核心 ID，移除保底 IP) ---
let id = 'e8c1580c-c3ae-4b51-99ea-09b16aa5cf02'; 

// --- 页面显示变量 (必须定义，否则页面报错) ---
let fname = '5pWw5a2X5aWX5Yip';   
let pName = '5pWw5a2X5aWX5Yip';   
let ytName = 'aHR0cHM6Ly95b3V0dWJlLmNvbQ=='; 
let tgName = 'aHR0cHM6Ly90Lm1l';             
...
let cachedIPs = [];
let lastFetchTime = 0;
// 全局声明变量，但不赋初始值，完全等 getDynamicProxy 来填充
globalThis.paddr = '';
globalThis.pnum = '';
...
async function getDynamicProxy() {
    const now = Date.now();
    // 缓存限流：1分钟
    if (cachedIPs.length === 0 || (now - lastFetchTime) > 60000) {
        try {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 5000);
            const response = await fetch(`${secretUrl}?t=${now}`, { signal: controller.signal });
    ...
    // 强制校验：如果 GitHub 没数据，直接报错，不走保底
    if (cachedIPs.length === 0) {
        throw new Error("No Proxy IP found in GitHub library.");
    }

    const selected = cachedIPs[Math.floor(Math.random() * cachedIPs.length)];
    const [addr, port] = selected.split(':');
    return { addr, port: port || '443' };
}

export default {
    async fetch(request, env, ctx) {
        try {
            // 获取最新反代，若库失效此处会直接触发 catch 块抛出 500 错误
            const dynamic = await getDynamicProxy();
            let currentPaddr = dynamic.addr;
            let currentPnum = dynamic.port;
            ...
            // 环境变量/URL 参数保留最高覆盖权（用于紧急调试）
            let manualPaddr = url.searchParams.get('PADDR') || PADDR;
            if (manualPaddr) {
                const [ip, port] = manualPaddr.split(':');
                currentPaddr = ip;
                currentPnum = port || currentPnum;
            }

            // 注入全局供给连接器使用
            globalThis.paddr = currentPaddr;
            globalThis.pnum = currentPnum;
            ...
            switch (url.pathname.toLowerCase()) {
                case '/': return await login(request, env);
                case `/${id}/get`: return get_kv(env);
                case `/${id}/set`: return set_kv_data(request, env);
                default: return Response.redirect(new URL('/', request.url));
            }
        } catch (err) {
            // 如果 GitHub 没给 IP，客户端会收到这个 500 错误
